...
import { Bundle, Pair, PairDayData, Token, TokenDayData, ZuniswapDayData, ZuniswapFactory } from '../types/schema'
import { PairHourData } from './../types/schema'
import { FACTORY_ADDRESS, ONE_BI, ZERO_BD, ZERO_BI } from './helpers'

export function updateUniswapDayData(event: EthereumEvent): ZuniswapDayData {
  let uniswap = ZuniswapFactory.load(FACTORY_ADDRESS)
  let timestamp = event.block.timestamp.toI32()
  let dayID = timestamp / 86400
  let dayStartTimestamp = dayID * 86400
  let uniswapDayData = ZuniswapDayData.load(dayID.toString())
  if (uniswapDayData === null) {
    uniswapDayData = new ZuniswapDayData(dayID.toString())
  ...
  return uniswapDayData as ZuniswapDayData
}
